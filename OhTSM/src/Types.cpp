...
{
	YLevel 
		YLevel::MAX(std::numeric_limits< signed short >::max()),
		YLevel::MIN(std::numeric_limits< signed short >::lowest());

	YLevel::YLevel( const signed short nNumericalRepresentation ) 
		: _position(nNumericalRepresentation) {}

	YLevel::YLevel() 
		: _position(0) {}

	YLevel::YLevel( YLevel && move ) 
